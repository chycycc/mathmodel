...
def read_result_csv(filename, required=True):
    """从代码结果目录读取 CSV；尚未求解的问题允许保留模板空表。"""
    path = os.path.join(RESULTS_DIR, filename)
    if not os.path.exists(path):
        if required:
            raise FileNotFoundError(f"缺少必需结果文件: {path}")
        print(f"跳过 {filename}：文件尚未生成，保留对应模板表头")
        return None
...
def export_all_results():
    ...
    df_q1 = read_result_csv('Q1_单点组批方案.csv')
    ...
    df_q2_trips = read_result_csv('Q2_运输架次.csv')
    ...
    df_q2_boxes = read_result_csv('Q2_逐箱交付.csv')
    ...
    df_q3_relays = read_result_csv('Q3_中继架次.csv', required=False)
    if df_q3_relays is not None:
    ...
    df_q3_comm = read_result_csv('Q3_通信保障.csv', required=False)
    if df_q3_comm is not None:
    ...
    df_q4 = read_result_csv('Q4_分区配置.csv', required=False)
