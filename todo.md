...
## 阶段二：问题三与问题四求解（待后续执行）
- [ ] **问题三：通信中继联合调度**
  - [ ] 读取 30m DEM 矩阵，计算 29 个运输架次的三维空间航迹
  - [ ] 计算与 G01 的视距遮挡（LOS）及 122 dB 链路通断情况
  - [ ] 求解中继无人机（R01, R02）悬停位置与轮换时序
  - [ ] 导出 `results/Q3_中继架次.csv` 与 `results/Q3_通信保障.csv`
- [ ] **问题四：任务分区与资源配置**
  - [ ] 施加跨服务区多点航次不可拆分约束
  - [ ] 完成 15 个服务区在 $K=2$ 与 $K=3$ 时的任务组划分
  - [ ] 统计各组所需无人机、电池及中继资源的最大并发需求
  - [ ] 导出 `results/Q4_分区配置.csv`
- [ ] **导出 Excel 提交文件**
