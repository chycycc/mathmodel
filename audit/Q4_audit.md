...
|:---:|:---|:---|:---|:---:|
| 1 | K=2 服务区完备性与唯一归属 | 15个服务区每个必须且只能属于一个任务组，无重复遗漏 | 覆盖服务区数=15/15, 无重复无遗漏 | **通过** |
| 2 | K=3 服务区完备性与唯一归属 | 15个服务区每个必须且只能属于一个任务组，无重复遗漏 | 覆盖服务区数=15/15, 无重复无遗漏 | **通过** |
| 3 | K=2 同架次多服务区强绑定约束 | 同一运输架次涉及多个服务区时，这些服务区必须划入同一任务组 | 全24个架次无任何跨组撕裂现象，多点航线100%组内封闭 | **通过** |
| 4 | K=3 同架次多服务区强绑定约束 | 同一运输架次涉及多个服务区时，这些服务区必须划入同一任务组 | 全24个架次无任何跨组撕裂现象，多点航线100%组内封闭 | **通过** |
| 5 | 任务组非空约束 | 每个任务组至少包含一个服务区 | K=2组包含[13, 2]个服务区; K=3组包含[13, 1, 1]个服务区，全部非空 | **通过** |
| 6 | 资源配置峰值并发核算与缺口计算 | 严格基于各组流水线峰值与两阶段充电时间核算各型飞机、电池与中继缺口 | 完成全部 8 类装备（A/B/C飞机、A/B/C电池、中继机、组件）独立需求与缺口精准核算 | **通过** |
