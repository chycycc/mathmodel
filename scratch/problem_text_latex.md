...
本问题在不考虑实体无人机和共享电池调度的情况下，研究单服务区直接往返条件下的运输能力与货箱组批问题。每个架次采用 O01→Si→O01 的直接往返形式，仅服务一个服务区，其中 $O01$ 表示临时调度中心，Si表示第i个服务区（ $i=1,2,…,15$ ）；同一服务区可由多个架次分批服务，不得跨服务区组批。根据附件给出的节点坐标、30 米 DEM、机型参数和货箱数据，完成如下各项：
...
同时，讨论返航安全余量变化对不同机型最大安全载荷和货箱组批结果的影响。
...
综合考虑配送及时性、全部任务完成时间、运输能耗和架次数等指标进行优化，并说明各指标之间的权衡关系。（全部任务完成时间取所有运输无人机完成最后一个架次并返回O01 的最晚时刻。）
...
综合考虑物资配送及时性、联合任务完成时间、运输与中继总能耗以及两类无人机的使用架次等指标进行优化，并说明各指标之间的权衡关系。（其中，联合任务完成时间取所有运输无人机和中继无人机完成最后一个架次并返回 O01 的最晚时刻。）
...
分别给出划分为2个和3个任务组时的任务分区及资源配置方案，并从资源配置规模、资源冗余、组间工作量均衡以及与现有库存之间的资源缺口等方面进行比较，分析两种分区方式的特点。
...
${L}_{g}(q)={L}_{g}^{0}-({L}_{g}^{0}-{L}_{g}^{F}){(\frac{q}{{Q}_{g}})}^{3/2},  0≤q≤{Q}_{g}.$

其中， $g$  表示运输无人机机型， $q$  表示当前有效载荷（kg）； ${L}_{g}^{0}$ 和 ${L}_{g}^{F}$ 分别表示机型  $g$  的空载标准航程和满载标准航程（m）； ${Q}_{g}$  表示机型  $g$  的最大载货质量（kg）。

航段的飞行时间为：

${t}_{gij}=\frac{{h}_{ij}^{+}}{{v}_{g}^{↑}}+\frac{{d}_{ij}}{{v}_{g}^{c}}+\frac{{h}_{ij}^{-}}{{v}_{g}^{↓}}$

其中， ${t}_{gij}$  表示机型  $g$  从节点  $i$  到节点  $j$  的航段飞行时间（s）； ${d}_{ij}$  为水平巡航距离（m）； ${h}_{ij}^{+}$ 、 ${h}_{ij}^{-}$  分别为爬升高度和下降高度（m）； ${v}_{g}^{↑}$ 、 ${v}_{g}^{c}$ 、 ${v}_{g}^{↓}$  分别为爬升、巡航和下降速度（m/s）。

航段运输能耗按水平航程能耗与爬升附加能耗之和计算：

${E}_{gij}(q)={E}_{gij}^{hor}(q)+{E}_{gij}^{up}(q).$

其中， ${E}_{gij}(q)$  为航段总运输能耗， ${E}_{gij}^{hor}(q)$  和  ${E}_{gij}^{up}(q)$  分别为水平巡航能耗和爬升附加能耗（kWh）。

下降能耗效率取0表示不单独计算下降附加能耗。每个运输架次均须满足返航安全余量：

${E}_{p}^{T}=(i,j)∈p​{E}_{gij}({q}_{pij})≤(1-{ρ}_{g}){E}_{g}^{use}.$

其中， ${E}_{p}^{T}$  为架次  $p$  的总运输能耗（kWh）， ${q}_{pij}$  为架次  $p$  在航段  $i$ → $j$  上的剩余载荷（kg）， ${ρ}_{g}$  为机型  $g$  的返航安全余量比例， ${E}_{g}^{use}$  为机型  $g$  单组电池的可用能量（kWh）。
...
${t}_{chg}(s) =({T}_{full}[0.65(0.90 - s)/0.90 + 0.35],    0 ≤ s < 0.90{T}_{full}·0.35(1 - s)/0.10,    0.90 ≤ s ≤ 1)$

其中， $s$ 为电池或能源组件任务结束时的SOC， ${t}_{chg}(s)$ 表示该资源从任务结束时 SOC 为  $s$  的状态充电至100%所需的时间， ${T}_{full}$ 为相应资源的等效完全充电时间。同一资源的任务占用与充电时段不得重叠，不同资源可并行充电。
...
${b}_{ij}(t)=(0,端点i与j之间不存在地形遮挡,1,端点i与j之间存在地形遮挡)$

（2）接收门限与双向链路预算

有效接收门限由接收灵敏度和衰落裕量共同确定：

${P}_{th}^{b}={P}_{sens}^{b}+{M}^{b}$

其中， ${P}_{sens}^{b}$  和  ${M}^{b}$  分别为接收端  $b$  的接收灵敏度和衰落裕量，具体取值见“通信链路参数.xlsx”。

对于通信方向  $a→b$ ，最大允许总传播损耗为

${L}_{max}^{a→b}={P}_{t}^{a}+{G}_{t}^{a}+{G}_{r}^{b}-{L}_{sys}-{P}_{th}^{b}$

其中， ${P}_{t}^{a}$  和  ${G}_{t}^{a}$  分别为发射端  $a$  的发射功率和发射天线增益， ${G}_{r}^{b}$  为接收端  $b$  的接收天线增益， ${L}_{sys}$  为系统损耗。

由于运输控制与状态回传均需要通信保障，主体  $a$  与  $b$  之间按照双向链路进行判定，其链路门限取两个通信方向最大允许传播损耗中的较小值：

${L}_{max}^{a↔b}=min({L}_{max}^{a→b},{L}_{max}^{b→a})$

（3）传播损耗计算

端点  $i$  与  $j$  在时刻  $t$  的自由空间传播损耗为

${L}_{FSPL,ij}(t)=32.45+20{log}_{10}f+20{log}_{10}{D}_{ij}(t)$

其中，载波频率  $f$  以 MHz 计， ${D}_{ij}(t)$  为两个通信端点之间的三维直线距离，以 km 计。

考虑地形遮挡后的总传播损耗为

${L}_{path,ij}(t)={L}_{FSPL,ij}(t)+{L}_{obs}{b}_{ij}(t)$

其中， ${L}_{obs}$  为地形遮挡附加损耗，具体取值见“通信链路参数.xlsx”。

（4）单链路可用性判定

端点  $i$  与  $j$  之间的双向链路可用性定义为

${A}_{ij}(t)=(1,{L}_{path,ij}(t)≤{L}_{max}^{i↔j},0,{L}_{path,ij}(t)>{L}_{max}^{i↔j})$

其中， ${A}_{ij}(t)=1$  表示相应链路可用， ${A}_{ij}(t)=0$  表示相应链路不可用。
