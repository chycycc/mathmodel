...
| :---: | :--- | :--- | :---: | :---: | :--- |
| **图 1** | `fig1_terrain_rescue_network` | **山区高精度 2D GIS 矢量等高线测绘与应急救援网络拓扑图**<br>*High-Precision 2D GIS Topographic Contours and Drone Rescue Network Topology* | 第 1 章 / 第 2 章 背景与假设 | 双栏 (7.2 in) | **【高精度 2D 矢量测绘升级】** 基于 30m DEM 矩阵构建高精度矢量等高线测绘底图（40m 等高距自然植被大地色阶，首曲线与计曲线精准注记，彻底杜绝模糊与伪 3D）；右下角增设 $0-2-4\text{ km}$ 地学标准黑白比例尺；受灾人口四级离散分级（尺寸与色彩双重冗余编码），S001（2100人）配置专属一级核心主干航路实线，S009 航线沿东侧山谷平滑分流，全网 15 条生命线清晰独立、无一遮挡。 |
...
| **图 5** | `fig5_q1_eta_sensitivity` | **返航安全电量余量 $\eta$ 敏感性多指标全景分析图**<br>*Multi-objective Sensitivity Analysis of Reserve Power Margin $\eta$* | 第 3 章 问题一敏感性分析 | 双栏 (7.2 in) | Panel (a) 揭示架次构成随 $\eta$ 的阶梯变化（$\eta \le 0.20$ 稳定在 18 架次）；Panel (b) 揭示 $\eta > 0.20$ 后总能耗与作业时间呈现阶梯式显著增加，论证 $\eta=0.20$ 为安全性与运力之间的基准平衡点。 |
| **图 6** | `fig6_q2_alns_convergence_comparison` | **ALNS 启发式求解收敛轨迹与邻域算子自适应演化**<br>*ALNS Convergence Trajectory and Adaptive Operator Weight Evolution* | 第 4 章 问题二算法设计 | 双栏 (7.2 in) | Panel (a) 呈现 Seed 42, 43, 44 真实日志收敛过程，Seed 42 在第 56 代锁定综合目标值 424.79（24架次，78.27 kWh）；探索解散点严守下包络线公理；Panel (b) 展示 6 大破坏-修复算子从 16.7% 均等初始权重自适应动态演进。 |
...
| **图 10** | `fig10_q3_los_dem_profile_blockage` | **30m DEM 山区复杂地形视距遮挡 (LOS) 射线追踪与第一菲涅尔区保活几何机制**<br>*3D Terrain Ray-Casting for LOS Blockage and 1st Fresnel Zone Preservation* | 第 5 章 问题三信道建模 | 双栏 (7.2 in) | **【维度三升级】** 引入 $f=2.4\text{ GHz}$ 第一菲涅尔区（$F_1$）空间信道半透明椭球透视带，直观揭示 O01 $\to$ S003 沿线最高山脊（高程 423.8m）刺入第一菲涅尔区造成的严重深衰落阻断，严密证明高空中继机 RT01（悬停 731.6m）架设的物理必然性。 |
